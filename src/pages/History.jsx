...
export default function History() {
  return (
    <section className="bg-gradient-to-b from-[#0a1123] to-[#111827] text-white overflow-hidden">
      ...
      <header className="relative py-16 px-8 md:px-24 text-center border-b border-amber-300/50 bg-cover bg-center" style={{ backgroundImage: `url(${Periodico})` }}>
        <div className="absolute inset-0 bg-[#0a1930]/80 backdrop-blur-[2px]"></div>
        <motion.div className="relative z-10" initial={{ opacity: 0, y: -40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1 }}>
          ...
          <p className="mt-3 text-lg italic text-gray-200">La historia y la arquitectura de Santa Fé de Antioquia</p>
        </motion.div>
      </header>

      {/* Sección video + texto */}
      <section className="relative z-10 grid grid-cols-1 md:grid-cols-3 gap-10 mt-16 px-8 md:px-24 items-center">
        <motion.div className="flex justify-center" initial={{ opacity: 0, x: -60 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 1 }}>
        ...
        <motion.article className="col-span-2 bg-[#00000050] backdrop-blur-sm p-6 rounded-xl border border-amber-200/30" initial={{ opacity: 0, x: 60 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 1 }}>
      ...
      <motion.div className="px-[20%] my-12" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 1.2 }}>
      ...
      <section className="relative z-10 px-6 md:px-24">
        <motion.h2 className="text-3xl md:text-4xl font-[Playfair_Display] mb-8 text-center text-amber-300 uppercase tracking-wide" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 1 }}>
      ...
      {/* Materiales */}
      <motion.section className="relative z-10 bg-[#00000070] mx-8 md:mx-24 my-16 p-10 rounded-2xl border border-amber-300/30" initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} transition={{ duration: 1 }}>
        ...
        <div className="p-10 grid gap-10 md:grid-cols-2">
          <Card
            titulo="Tapia Pisada y Bahareque"
            img={CasaBareque}
            parrafos={[
              "Los muros de tapia pisada fueron comunes en las casas coloniales de Santa Fe de AntioquiaEn las viviendas domésticas coloniales y vernáculas, se empleaban muros de tapia pisada (tierra compactada en moldes / encofrados) o de bahareque (estructura de madera o cañas, relleno de tierra, paja o mezclas similares) como sistemas principales de cerramiento.",
              "Los cimientos solían ser de piedra ciclópea, luego se levantaban muros de tapia o bahareque, y sobre éstos se asentaban los techos de teja de barro cocido.",
              "Este sistema respondía al clima cálido-tropical de la región, a la disponibilidad de materiales locales (tierra, caña, madera) y a la tradición española adaptada al nuevo mundo.",
            ]}
            referencias="Fuente: https://es.scribd.com/document/457790963/MATERIALES-Y-TECNICAS-CONSTRUCTIVAS?"
          />

          <Card
            titulo="Teja de Barro y Madera"
            img={CasaMadera}
            parrafos={[
              "Los techos se cubrían con tejas de barro cocido, que permitían ventilación, enfriamiento y resistencia a la lluvia.",
              "Para portadas, arcos y elementos decorativos de mayor categoría, se usaba el ladrillo como material básico visible: jambas, arquivoltas, dinteles y capiteles decorados.",
              "La madera tenía un papel destacado: ventanas, puertas, balcones, chambranas y carpintería representaban la artesanía local y eran elementos estéticos y funcionales en las edificaciones.",
            ]}
            referencias="Fuente: https://elturismoencolombia.com/a-donde-ir/turismo-santa-fe-antioquia/construcciones-coloniales-centro-historico-de-santa-fe-de-antioquia/?"
          />

          <Card
            titulo="Piedra, mortero y acabados orgánicos"
            img={CasaDecoraciones}
